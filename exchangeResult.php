...
<body>
    ...
    <main class=" max-w-[1000px] mx-auto p-10">
        ...
        <section class=" bg-gray-100 p-10 rounded-lg">
            ...
            <p class="lg:text-4xl text-lg sm:text-2xl text-center mb-5">
                <?php
                ...
                $a = empty($_POST['base_amount']);
                $b = empty($_POST['base_currency']);
                $t = empty($_POST['target_currency']);
                ...
                if (array_key_exists($baseCurrency, $currencies)) {
                    $baseValue = $currencies[$baseCurrency];
                    $numericValue = (float)str_replace(',', '', $baseValue);

                    $amountInMMK = $amount * $numericValue;
                    echo  $amount . $baseCurrency . " to " . " MMK " . " = " . $amountInMMK . "<br>";

                    if (array_key_exists($targetCurrency, $currencies)) {
                        $targetValue = $currencies[$targetCurrency];
                        $targetNumeric = (float)str_replace(',', '', $targetValue);

                        $result = $amountInMMK / $targetNumeric;
                        $final = number_format($result, 2, '.', '');
                        echo $amount . $baseCurrency . " to " . $targetCurrency . " = " . $final;

                        $exchangeRecord = "saveExchange.txt";

                        if (!file_exists($exchangeRecord)) {
                            touch($exchangeRecord);
                        }

                        $fileStream = fopen($exchangeRecord, 'a');
                        fwrite($fileStream, "$amount $baseCurrency to MMK = $amountInMMK / $amount $baseCurrency to $targetCurrency = $final \n");
                        fclose($fileStream);
                    }
                } else {
                    echo "Key $baseCurrency not found in the array";
                }
            ...
            <div class="flex w-full gap-3">
                <a href="exchangeRate.php" type="button" class="py-3 flex-grow text-center justify-center  px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                    Calculate again
                </a> <a href="exchangeRecord.php" type="button" class="py-3 flex-grow text-center justify-center  px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                    Show Records
                </a>
            </div>
